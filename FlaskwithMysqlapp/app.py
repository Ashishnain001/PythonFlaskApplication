...
handler_application = RotatingFileHandler('applications.log')
logger.addHandler(handler_application)

logger_flask = logging.getLogger('werkzeug')
logger.setLevel(logging.DEBUG)
handler_flask = RotatingFileHandler('access.log')
logger_flask.addHandler(handler_flask)
# Initialisation
app = Flask(__name__,
            static_url_path='',
...
app.config['MYSQL_PASSWORD'] = 'Cavisson@123'
...
# Index code start
@app.route('/')
@app.route('/login/id=1045?name=Ashish', methods=['GET', 'POST'])
def login():
 #   print("inside login")
  #  print("Login start")
   # print("*************************************************************")
...
def http_page():
    ...
    logger.critical("Critical log info")
    #print("\n\nhttp page\n\n")
    #r = requests.get("https://www.bing.com/se/arch?q=a")
    r = requests.get("http://172.24.0.228:1234/tiny")
    #r = requests.get("http://10.10.30.28:8080/tiny")
...
def getuserdetails():
    ...
    #atexit.register(goodbye, 'Ashish', 'nice')
    #print('in dummy 2****************************************************************')
